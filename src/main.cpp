	...
	while (true)
	{
		...
		if (params.Process && progRun)
		{
			...
			if (!frame.empty())
			{
				...
				if(params.Debug)
				{
					cout << "Target Dist:" << targets.targetDistance << endl << endl;
				}
...
void findTarget(Mat original, Mat thresholded, Target& targets, const ProgParams& params)
{
	...
	cout << "Hierarchy: " << hierarchy.size() << endl << endl;
	}
	...
	if (!contours.empty() && !hierarchy.empty())
	{
		...
		for (unsigned int i = 0; i < contours.size(); i++)
		{
			...
			if (area > MAX_SIZE) {
				if (targets.Width < box.width) {
					targets.Width = box.width;
					targets.FoundTarget = box;
...
void *TCP_Send_Thread(void *args)
{
	...
	while (true)
	{
		...
		//TODO MESSAGE TO SEND TO SERVER
		message << "CONSTRUCT IT" << endl;
...
//		pthread_mutex_lock(&matchStartMutex);
		...
		//TODO WHAT SHOULD THE SERVER SEND TO THE TEGRA IF ANYTHING
...
void *VideoCap(void *args)
{
	...
	if (struct_ptr->From_File)
	{
		...
		pthread_mutex_lock(&frameMutex);
		frame = imread(struct_ptr->IMAGE_FILE, 1);
		pthread_mutex_unlock(&frameMutex);
